...
class UserRecord(Record):
    ...
    def verify_id_token(id_token, check_revoked, app):
        try:
            ...
            raise abort(
                401, description="ExpiredIdToken", response="auth/id-token-expired"
            )
        except auth.RevokedIdTokenError as ex:
            raise abort(
                401, description="RevokedIdToken", response="auth/id-token-revoked"
            )
        except auth.InvalidIdTokenError as ex:
            raise abort(
                401, description="InvalidIdToken", response="auth/invalid-id-token"
            )
            ...
            raise abort(
                500, description="UnexpectedResponse", response="unexpected-response"
            )
    ...
    def create_user(cls, user, app):
        try:
            ...
            raise abort(
                400, description="UidAlreadyExists", response="auth/uid-already-exists"
            )
